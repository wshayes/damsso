...
from .models import Tenant, TenantUser, SSOProvider
...
class MultiTenantAccountAdapter(DefaultAccountAdapter):
    ...
    def is_open_for_signup(self, request):
        """
        Allow signup only if user has a valid invitation or tenant allows it.
        """
    ...
    def pre_authenticate(self, request, **credentials):
        ...
        if email:
            ...
            if tenant_user:
                ...
                messages.warning(
                    request,
                    _("Your organization requires Single Sign-On. Please use the SSO login.")
                )
    ...
    def save_user(self, request, user, form, commit=True):
        """
        Save user and associate with tenant if invitation exists.
        """
            ...
            if invitation_token:
                ...
                try:
                    ...
                    if invitation.is_valid():
                        ...
                        messages.success(
                            request,
                            _("You have successfully joined {tenant}").format(
...
class MultiTenantSocialAccountAdapter(DefaultSocialAccountAdapter):
            ...
            if tenant_id:
                try:
                    ...
                    if created:
                        messages.success(
                            request,
                            _("Successfully joined {tenant}").format(tenant=tenant.name)
                        )
                        ...
                        messages.success(
                            request,
                            _("Your account has been reactivated for {tenant}").format(
                                tenant=tenant.name
                            )
                        )

                    # Store external ID if provided
                    external_id = sociallogin.account.extra_data.get('sub') or \
    ...
    def get_connect_redirect_url(self, request, socialaccount):
        ...
        return getattr(
            settings,
            'MULTITENANT_ACCOUNT_CONNECT_REDIRECT_URL',
            reverse('account_connections')
        )
