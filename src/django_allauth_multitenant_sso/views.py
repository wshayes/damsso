...
def tenant_login(request, tenant_slug):
    ...
    if tenant.sso_enforced and tenant.sso_enabled and sso_provider:
        return redirect("allauth_multitenant_sso:sso_login", tenant_slug=tenant_slug)
            ...
            if user is not None:
                ...
                try:
                    ...
                    next_url = request.GET.get("next") or reverse(
                        "allauth_multitenant_sso:tenant_dashboard", args=[tenant_slug]
                    )
    ...
    return render(request, "allauth_multitenant_sso/tenant_login.html", context)
...
def tenant_logout(request, tenant_slug):
    ...
    return redirect("allauth_multitenant_sso:tenant_login", tenant_slug=tenant_slug)
...
def oidc_callback(request, tenant_slug):
    ...
    try:
        client = OIDCProviderClient(sso_provider)
        redirect_uri = request.build_absolute_uri(reverse("allauth_multitenant_sso:oidc_callback", args=[tenant_slug]))
...
def manage_users(request, tenant_slug):
    ...
    if request.headers.get("HX-Request"):
        return render(request, "allauth_multitenant_sso/_user_list.html", context)

    return render(request, "allauth_multitenant_sso/manage_users.html", context)
...
def manage_sso_provider(request, tenant_slug):
    ...
    saml_acs_url = request.build_absolute_uri(reverse("allauth_multitenant_sso:saml_acs", args=[tenant.id]))
    saml_metadata_url = request.build_absolute_uri(reverse("allauth_multitenant_sso:saml_metadata", args=[tenant.id]))
...
def test_user_sso_login(request, tenant_slug):
    ...
    return render(request, "allauth_multitenant_sso/test_user_sso.html", context)
...
def _initiate_test_sso_login(request, tenant, expected_email):
    ...
    if not tenant.sso_enabled:
        ...
        return redirect("allauth_multitenant_sso:test_user_sso", tenant_slug=tenant.slug)
    ...
    return redirect("allauth_multitenant_sso:sso_login", tenant_slug=tenant.slug)
...
def tenant_signup(request, token):
    ...
    if request.user.is_authenticated:
        ...
        return redirect("allauth_multitenant_sso:tenant_dashboard", tenant_slug=tenant.slug)
...
def accept_invitation(request, token):
    ...
    if request.user.is_authenticated and request.user.email == invitation.email:
        try:
            ...
            return redirect("allauth_multitenant_sso:tenant_dashboard", tenant_slug=invitation.tenant.slug)
        except Exception as e:
...
def _handle_test_mode_callback(request, tenant, expected_email, actual_email, test_admin_user_id, test_tenant_slug):
    ...
    return redirect("allauth_multitenant_sso:test_user_sso", tenant_slug=test_tenant_slug or tenant.slug)
